...
%--------------------------------------------------------------------------
    ...
    [N,M]  = size(PopObj);
    fmin   = min(min(PopObj,[],1),zeros(1,M));%min(PopObj,[],1)，包含每一列的最小值的行向量，
    fmax   = max(PF,[],1);
    PopObj = (PopObj-repmat(fmin,N,1))./repmat((fmax-fmin)*1.1,N,1);%repmat（fmin,N,1）:重复N*1个fmin副本
    PopObj(any(PopObj>1,2),:) = [];%any(PopObj>1,2)测试每一行中是否有大于1的值
